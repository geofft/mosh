         ...
         char *envp[])
{
  ...
  if (!(child_termios.c_iflag & IUTF8)) {
    fprintf(stderr, "Warning: Locale is UTF-8 but termios IUTF8 flag not set. "
                    "Setting IUTF8 flag.\n");
    child_termios.c_iflag |= IUTF8;
  }
#else
  fprintf(stderr, "Warning: termios IUTF8 flag not defined. Character-erase of "
                  "multibyte character sequence probably does not work "
...
int vt_parser(int fd, Parser::UTF8Parser *parser)
{
  ...
  for (int i = 0; i < bytes_read; i++) {
    ...
    for (std::list<Parser::Action *>::iterator j = actions.begin();
      ...
      if (act->char_present) {
        if (iswprint(act->ch)) {
          printf("%s(0x%02x=%lc) ", act->name().c_str(), (unsigned int)act->ch,
                 act->ch);
        } else {
