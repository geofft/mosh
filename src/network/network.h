...
namespace Network
{
  ...
  public:
    string function;
    int the_errno;
    NetworkException(string s_function, int s_errno)
  ...
  public:
    ...
    Packet(uint64_t s_seq, Direction s_direction, uint16_t s_timestamp,
           uint16_t s_timestamp_reply, string s_payload)
        : seq(s_seq), direction(s_direction), timestamp(s_timestamp),
  ...
  public:
    ...
    Connection(const char *key_str, const char *ip,
    ...
    static bool parse_portrange(const char *desired_port_range,
                                int &desired_port_low, int &desired_port_high);
  };
}
