...
namespace Network
{
  template <class MyState, class RemoteState> class Transport
  {
  private:
    ...
    RemoteState last_receiver_state; /* the state we were in when user last
                                        queried state */
  ...
  public:
    ...
    /* Find diff between last receiver state and current remote state, then
     * rationalize states. */
    ...
    bool shutdown_in_progress(void) const
    {
      return sender.get_shutdown_in_progress();
    }
    bool shutdown_acknowledged(void) const
    {
      return sender.get_shutdown_acknowledged();
    }
    bool shutdown_ack_timed_out(void) const
    {
      return sender.shutdown_ack_timed_out();
    }
    bool has_remote_addr(void) const
    {
      return connection.get_has_remote_addr();
    }

    /* Other side has requested shutdown and we have sent one ACK */
    bool counterparty_shutdown_ack_sent(void) const
    {
      return sender.get_counterparty_shutdown_acknowledged();
    }
    ...
    uint64_t get_remote_state_num(void) const
    {
      return received_states.back().num;
    }

    const TimestampedState<RemoteState> &get_latest_remote_state(void) const
    {
      return received_states.back();
    }
    ...
    {
      return connection.get_send_exception();
    }
  };
}
